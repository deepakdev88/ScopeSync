...
const ClientView = () => {
  ...
  const fetchProjectData = useCallback(async (showToast = false) => {
    try {
      ...
      if (showToast) toast.error("Couldn't refresh. Try again.");
    } finally {
      setLoading(false)
      setIsSyncing(false)
    }
  ...
  const getProjectHealth = () => {
    ...
    if (percentage >= 50) {
      return {
        label: "On Track",
        color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20",
        desc: "More than half the tasks are done."
      };
    }
  ...
  if (!currentClientProject) {
    return (
      <div className="w-full min-h-screen bg-[#090a0f] flex flex-col gap-3 items-center justify-center font-mono text-xs text-gray-500 px-4 text-center">
        ...
        <h3 className="text-gray-300 font-bold text-sm">Project Not Found</h3>
        <p className="max-w-xs text-gray-600 leading-normal">This link is invalid or the project no longer exists.</p>
      </div>
    )
  }
        ...
        <div className="w-[90%] max-w-6xl mx-auto mt-10 p-6 bg-[#0d0e14]/50 border border-white/6 backdrop-blur-xl rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.5)] relative z-10 animate-fade-in flex flex-col gap-5">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/3 pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="text-[10px] font-bold tracking-widest text-gray-500 uppercase font-mono">Live</p>
                  <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse"></span>
                </div>
                <h2 className="text-lg font-extrabold text-white tracking-tight">Project Status</h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-center">
              ...
              <div className="md:col-span-3 space-y-3">
                <div className="flex justify-between items-end">
                  <h3 className="text-base font-bold text-gray-200 tracking-tight">
                    {completedTasks} <span className="text-xs font-mono font-normal text-gray-500">/ {totalTasks} Tasks Completed</span>
                  </h3>
              ...
              <div className="w-full p-3 rounded-xl bg-white/1 border border-white/4 flex flex-col gap-1 text-left font-mono min-h-21.25 justify-center">
                <span className="text-[9px] text-gray-600 uppercase tracking-widest">Status</span>
